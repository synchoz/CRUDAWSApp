...
- movies - Code for the application's Lambda function.
...
```bash
CRUDAWSSamApp$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
CRUDAWSSamApp$ sam local start-api
CRUDAWSSamApp$ curl http://localhost:3000/
```
...
```bash
CRUDAWSSamApp$ sam logs -n HelloWorldFunction --stack-name "crudawssamapp" --tail
```
...
```bash
CRUDAWSSamApp$ pip install -r tests/requirements.txt --user
# unit test
CRUDAWSSamApp$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
CRUDAWSSamApp$ AWS_SAM_STACK_NAME="crudawssamapp" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "crudawssamapp"
```
